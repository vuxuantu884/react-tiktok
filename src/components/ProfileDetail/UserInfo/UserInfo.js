...
function UserInfo({ userInfo = [], onToggleFollow = () => {}, onReport = () => {} }) {
  const menus = useRef([
    {
      title: 'Send message',
    ...
    <div className={styles.userInfo}>
      <div className={styles.shareInfo}>
        ...
        <div className={styles.titleContainer}>
          ...
          <h2 className={styles.subTitle}>{`${userInfo.first_name} ${userInfo.last_name}`}</h2>
              ...
              !(currentUser?.id === userInfo.id) && (
                <div className={styles.followBox}>
                  {userInfo.is_followed ? (
                    ...
                    <Button size='l' type='primary' onClick={() => onToggleFollow(userInfo)}>
                      Follow
                    </Button>
                  )}
                </div>
      ...
      <div className={styles.countInfo}>
        <div className={styles.followingCount}>
          <strong className={styles.quantity}>{userInfo.followings_count}</strong>
          <span>Following</span>
        </div>
        <div className={styles.followerCount}>
          <strong className={styles.quantity}>{userInfo.followers_count}</strong>
          <span>Followers</span>
        </div>
        <div className={styles.likeCount}>
          <strong className={styles.quantity}>{userInfo.likes_count}</strong>
          <span>Likes</span>
        </div>
      </div>

      <h2 className={styles.shareDesc}>{userInfo.bio ? userInfo.bio : 'No bio yet'} 😉</h2>
